	...
	for invoice in invoices:
		...
		for item in invoice_items:
			...
			if invoice.is_return == 0:
				total_taxable_amount += get_tax_amount(item.item_code, vat_setting.account, doctype, invoice.name)

			if invoice.is_return == 1:
				total_taxable_adjustment_amount += get_tax_amount(item.item_code, vat_setting.account, doctype, invoice.name)
...
def append_data(data, title, account, amount, adjustment_amount, vat_amount, company_currency):
